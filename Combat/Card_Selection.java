...
public class Card_Selection
{
    public static int Selection(int counter)
    {
        ...
        if (counter<6)
        {
            ...
            do
            {
                ...
                good=false;
                if (Cname==616||Cname<=0||(Cname>7&&Cname!=9)) //update with each version
                {
    ...
    public static int Retry(boolean banned)
    {
        ...
        do
        {
            ...
            if (rename==616||rename<=0||(rename>7&&rename!=9))
            {
                System.out.println("Index number not found.");
            }
            else
    ...
    public static Character ChooseTargetFoe (Character hero, Character[] list)
    {
        ...
        if (team.size()>1) //the untargetable heroes are only untargetable if they are not alone on their team
        {
            for (Character c: team)
            {
                ...
                {
                    if (c.CheckFor(c, "Invisible")==true)
                    {
                        remove.add(c);
                    }
                }
