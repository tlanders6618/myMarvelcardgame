...
class BuffAb extends Ability
{
            ...
            for (Character chump: targets) //use the ability on its target
            {
                if (chump!=null) //if null, skip entirely
                {
                    ...
                    {  
                        ...
                        if (!(user.binaries.contains("Missed"))&&array[4].equalsIgnoreCase("true"))
                        {
                            selfapply.add(New);
                        }
                    ...
                    toadd=Ability.ApplyStats(user, chump, together, selfapply, otherapply); //applied directly since buff skills can't be used on enemies
                    for (SpecialAbility ob: special)
                    {
    ...
    {
        boolean okay=true;
        if (user.CheckFor(user, "Buff Disable")==true||user.CheckFor(user, "Ability Disable")==true)
        {
            okay=false;
        }
