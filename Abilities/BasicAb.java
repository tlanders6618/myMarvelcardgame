            ...
            for (Character chump: targets) //use the ability on its target
            {
                if (chump!=null) //if null, skip entirely to avoid null exception
                {
                    do 
                    {
                        ...
                        {
                            chump=user.AttackIgDefence (user, chump, damage, aoe);
                        }
                        else if (user.ignores.contains("Status effects"))
                        {
                            chump.HP-=damage;
                        }
                        else if (lose==true)
                        ...
                        --multi;
                        user.CheckDrain(user, damage);
                        damage=odamage; //reset damage 
                    }
                    while (multi>0); //then repeat the attack for each multihit
                    --uses;
