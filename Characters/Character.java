...
public class Character 
{
    ...
    int passivecount; //counter for passives like Nitro's
    ...
    public Character Attack (Character dealer, Character target, int dmg, boolean aoe)
    {
        ...
        if (evade==false&&nomiss==true)
        {
            ...
            dealer.activeability[0].ReturnDamage(dmg);
    ...
    {
        boolean evade=false;
        if (!(target.binaries.contains("Stunned"))&&(target.CheckFor(target, "Shatter")==false)) //things that would allow the damage to ignore evade
        {
            for (StatEff effect: target.effects) 
            {
                if (effect.getimmunityname().equalsIgnoreCase("Evade"))
        ...
        if (evade==false)
        {
            if (!(target.binaries.contains("Stunned")))
            {
                for (StatEff eff: target.effects)
                {
                    if (eff.getimmunityname().equalsIgnoreCase("Guard"))
            ...
            dmg=(dmg+target.DV)-(target.ADR+target.DR);
            if (dmg<0)
            {
                dmg=0;
            }
    ...
    public void DOTdmg (Character target, int dmg, String type) //target is the one taking the damage
    {
        ...
        {
            dmg=(dmg-ADR-BlDR); //factoring in damage resistance
    ...
    public static int InHP (int index, int health)
    {
        ...
        switch (index)
        {
            ...
            case 1: case 2: case 3: case 5: case 6: case 4: 
            health= 230;  break;
    ...
    public void onDeath (Character hero, Character killer, String dmgtype)
    {
        ...
        for (StatEff eff: hero.effects)
        {
            removeme.add(eff); //to undo their effects
        }        
    ...
    public StatEff GetRandomStatEff (Character hero, String type)
    {
